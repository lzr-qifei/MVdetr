...
class _BaseDataset(ABC):
    ...
    @_timing.time
    def get_raw_seq_data(self, tracker):
    ...
    @staticmethod
    # def _calculate_point_dist(pt1,pt2):
    #     dist_matrix = np.linalg.norm(pt1[:, np.newaxis] - pt2, axis=2)
    #     return dist_matrix
    def _calculate_point_dist(pt1, pt2):
    # 计算pt1和pt2每一对点之间的欧式距离
        dist_matrix = np.linalg.norm(pt1[:, np.newaxis] - pt2, axis=2)
        ...
        if max_dist > min_dist:
            normalized_dist = (dist_matrix - min_dist) / (max_dist - min_dist)
        else:
            normalized_dist = dist_matrix  # 如果所有距离相等，则无需归一化
