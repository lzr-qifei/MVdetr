...
class WildTrack(_BaseDataset):
    ...
    def get_default_dataset_config():
        ...
        default_config = {
            ...
            'SEQMAP_FOLDER': '',  # Where seqmaps are found (if None, GT_FOLDER/seqmaps)
            'SEQMAP_FILE': None,  # Directly specify seqmap file (if none use seqmap_folder/benchmark-split_to_eval)
            'SEQ_INFO': None,  # If not None, directly specify sequences to eval and their number of timesteps
            # 'GT_LOC_FORMAT': '{gt_folder}/{seq}/gt/gt.txt',  # '{gt_folder}/{seq}/gt/gt.txt'
            'GT_LOC_FORMAT': '{gt_folder}/{gt_loc}',
    ...
    def __init__(self, config=None):
        ...
        curr_file = os.path.join(self.tracker_fol,self.config['TRACKERS_TO_EVAL'][0])
        self.seq_list = []
    ...
    def get_display_name(self, tracker):
        # tracker_to_disp = self.config['']
        # return self.tracker_to_disp[tracker]
        return self.config['TRACKERS_TO_EVAL']
    ...
    def _load_raw_file(self, tracker, is_gt):
        ...
        current_time_keys = [str( t+ 5) for t in range(1795,1800+5*num_timesteps,5)]
        ...
        tmp_cnt = -1
        for t in range(1795,1795+5*num_timesteps,5):
            time_key = str(t+5)
            tmp_cnt+=1
            if time_key in read_data.keys():
                ...
                try:
                    ...
                    raw_data['dets'][tmp_cnt] = time_data[:, 2:4]
                    raw_data['ids'][tmp_cnt] = np.atleast_1d(time_data[:, 1]).astype(int)
                ...
                if time_data.shape[1] >= 8:
                    raw_data['classes'][tmp_cnt] = np.atleast_1d(time_data[:, 7]).astype(int)
                else:
                    if not is_gt:
                        raw_data['classes'][tmp_cnt] = np.ones_like(raw_data['ids'][tmp_cnt])
                    else:
                        raise TrackEvalException(
                ...
                if is_gt:
                    ...
                    raw_data['gt_extras'][tmp_cnt] = gt_extras_dict
                else:
                    raw_data['tracker_confidences'][tmp_cnt] = np.atleast_1d(time_data[:, 6])
            else:
                raw_data['dets'][tmp_cnt] = np.empty((0, 4))
                raw_data['ids'][tmp_cnt] = np.empty(0).astype(int)
                raw_data['classes'][tmp_cnt] = np.empty(0).astype(int)
                if is_gt:
                    gt_extras_dict = {'zero_marked': np.empty(0)}
                    raw_data['gt_extras'][tmp_cnt] = gt_extras_dict
                else:
                    raw_data['tracker_confidences'][tmp_cnt] = np.empty(0)
            if is_gt:
                raw_data['gt_crowd_ignore_regions'][tmp_cnt] = np.empty((0, 4))
    ...
    def _calculate_similarities(self, gt_dets_t, tracker_dets_t):
        ...
        similarity_scores = self._calculate_point_dist(gt_dets_t,tracker_dets_t)
        return similarity_scores

class MultiviewX(_BaseDataset):
    """Dataset class for MOT Challenge 2D bounding box tracking"""
    ...
    def _load_raw_file(self, tracker, is_gt):
        ...
        # Check for any extra time keys
        # current_time_keys = [str( t+ 1) for t in range(num_timesteps)]
        current_time_keys = [str( t+ 1) for t in range(359,359+num_timesteps)]
        extra_time_keys = [x for x in read_data.keys() if x not in current_time_keys]
        if len(extra_time_keys) > 0:
            if is_gt:
                text = 'Ground-truth'
        ...
        # for t in range(num_timesteps):
        tmp_cnt = -1
        for t in range(359,359+num_timesteps,):
            time_key = str(t+1)
            tmp_cnt+=1
            if time_key in read_data.keys():
                try:
