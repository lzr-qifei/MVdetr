...
from multiview_detector.models.matcher import HungarianMatcher,HungarianMatcher_batch,DETRMatcher
...
def main(config: dict):
    ...
    data_path = '/root/autodl-tmp/MultiviewX'
    base = MultiviewX(data_path)
    train_set = frameDataset(base, train=True, world_reduce=2,
                             img_reduce=12, world_kernel_size=10,
                             img_kernel_size=10, semi_supervised=0,
                             dropout=0, augmentation=True,train_ratio=0.9)
    test_set = frameDataset(base, train=False, world_reduce=2,
                             img_reduce=12, world_kernel_size=10,
                             img_kernel_size=10, semi_supervised=0,
                             dropout=0, augmentation=True,train_ratio=0.9)
    ...
    model = MVDeTr_w_dec(args=None,dataset=train_set).cuda()
    ...
    detr_matcher = DETRMatcher(cost_class=2,cost_pts=50)
    ...
    criterion = DETRCriterion(config['NUM_CLASSES'],detr_matcher,weight_dict,losses)
    ...
    for epoch in range(100):
        ...
        if config['RESUME'] is None:
            ...
            # )   
            # evaluate_one_epoch(config,model,test_loader,only_detr)
            print(f'epoch:{epoch}')
            # for i in range(10):
            # losses = train_one_epoch(config,model,logger,dataloader,id_criterion,criterion,
            #             optimizer,epoch,
            #             states,clip_max_norm,length_detr_train_frames,metrics,scheduler)
            losses = trainer.train_one_epoch(config,logger,train_loader,id_criterion,criterion,
                        optimizer,epoch,
                        states,clip_max_norm,length_detr_train_frames,metrics,scheduler)
            ...
            print(f'epoch:{epoch}')
            trainer.evaluate_one_epoch(config,test_loader,only_detr)
        else:
...
class Trainer():
    ...
    def train_one_epoch(self,config:dict, logger,dataloader:DataLoader,id_criterion:nn.Module,detr_criterion,
        ...
        # for idx,(imgs,affine_mats,info) in enumerate(dataloader):
            ...
            # frames = batch['images']
            frames = data.cuda(device=device)
            # infos = batch["infos"]
            ...
            cur_train_outputs = self.model(frames,affine_mats)
            
            detr_train_loss_dict,_ = detr_criterion(cur_train_outputs,world_gt)
            # if i == 5:
            ...
            print(f'idx:{batch_idx},frame:{frame},loss:{losses}')

        return losses.cpu()


    def evaluate_one_epoch_per_seq(config: dict,model: nn.Module,dataset,only_detr: bool = False):
        model.eval()
        device = config["DEVICE"]
    ...
    def evaluate_one_epoch(self,config: dict,dataloader:DataLoader,only_detr: bool = False):
        self.model.eval()
        device = config["DEVICE"]
        res_fpath = config['OUT_DIR']
        det_thres = 0.55
        ...
        with torch.no_grad():
            if only_detr :
                ...
                for batch_idx, (data, world_gt, imgs_gt, affine_mats, frame) in enumerate(dataloader):
                    ...
                    # for item in detr_no_grad_outputs:
                    frames = data.cuda(device=device)
                    outputs = self.model(frames,affine_mats)
                    if res_fpath is not None:
                        grid_xy,out_logits = outputs['pred_ct_pts'],outputs['pred_logits']
                        ...
                        pos_cpu[:,0] = pos_cpu[:,0]*1000
                        pos_cpu[:,1] = pos_cpu[:,1]*640
                        frame_idx = torch.ones([pos.shape[0], 1])* (batch_idx+360)
                        res = torch.cat([frame_idx, pos_cpu], dim=1)
                        res_list.append(res)
