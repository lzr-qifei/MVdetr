...
from statistic import MatrixHistogram
...
def speed_direction(pt1, pt2):
    # cx1, cy1 = (bbox1[0]+bbox1[2]) / 2.0, (bbox1[1]+bbox1[3])/2.0
    # cx2, cy2 = (bbox2[0]+bbox2[2]) / 2.0, (bbox2[1]+bbox2[3])/2.0
    cx1,cy1 = pt1[0] / 2.0, pt1[1] / 2.0
    cx2,cy2 = pt2[0] / 2.0, pt2[1] / 2.0
...
class KalmanBoxTracker(object):
    ...
    def __init__(self, point, delta_t=3, orig=False):
        """
            ...
            if self.last_observation.sum() >= 0:  # no previous observation
                ...
                for i in range(self.delta_t):
                    ...
                    if self.age - dt in self.observations:
                        previous_pt = self.observations[self.age-dt]
                        break
                if previous_pt is None:
                    previous_pt = self.last_observation[:-1]
                """
                  Estimate the track speed direction with observations \Delta t steps away
                """
                self.velocity = speed_direction(previous_pt, pt)
            
            """
              Insert new observations. This is a ugly way to maintain both self.observations
              and self.history_observations. Bear it for the moment.
            """
            # ob = np.concatenate(pt,score,axis=-1)
            self.last_observation = ob
            self.observations[self.age] = ob
            self.history_observations.append(ob)

            self.time_since_update = 0
            self.history = []
            self.hits += 1
            self.hit_streak += 1
            self.kf.update(convert_point_to_z(pt))
        else:
            self.kf.update(ob)
...
class OCSort_opps(object):
    ...
    def update(self, output_results):
        ...
        elif output_results.shape[1] == 3:
            scores = output_results[:,2]
            pts = output_results[:,:2]
        ...
        img_h,img_w = 640,1000
        # scale = min(img_size[0] / float(img_h), img_size[1] / float(img_w))
        # bboxes /= scale
        dets = np.concatenate((pts, np.expand_dims(scores, axis=-1)), axis=1)
        inds_low = scores > 0.1
        #这里是满足条件的所有检测
        ...
        if self.trackers !=[]:
            ...
            # self.histogram.add_matrix(dist_matrix)
            # if self.frame_count==39:
            #     path = '/home/lizirui/MVdetr/multiview_detector/tracker/OC_SORT/wild_results/dist.png'
            #     self.histogram.save_histogram(path)
        else:
            matched, unmatched_dets, unmatched_trks = associate(
                dets, trks, self.e_dist_threshold, velocities,
                k_observations, self.inertia)
        ...
        if self.use_byte and len(dets_second) > 0 and unmatched_trks.shape[0] > 0:
            ...
            if dist_left.min() < self.e_dist_threshold:
                ...
                for m in matched_indices:
                    ...
                    if dist_left[m[0], m[1]] > self.iou_threshold:
                        continue
        ...
        for i in unmatched_dets:
            trk = KalmanBoxTracker(dets[i, :-1], delta_t=self.delta_t)
